...
- Add a TSDoc/JSDoc comment to every exported function in `db/` and `src/lib/`. The comment must state the function's purpose, document every parameter (including the injectable `db` argument), and describe the return value. Keep comments focused on intent and behavior; do not paraphrase the implementation.
- Keep comments current when changing a helper, schema, transform, or migration workflow. Stale documentation is a correctness defect.
